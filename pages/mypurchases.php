...
$listSql = "
  SELECT 
    p.id AS order_id, p.created_at AS ordered_at, p.total_paid_cents, p.status,
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
      <div class="card rounded-xl border border-border bg-gradient-to-br from-blue-50 to-white p-5">
        <div class="flex items-center gap-4">
          ...
          <div><p class="text-slate-500 text-sm">Total Orders</p><p class="text-2xl font-bold"><?= $total_orders ?></p></div>
        </div>
      </div>
      <div class="card rounded-xl border border-border bg-gradient-to-br from-emerald-50 to-white p-5">
        <div class="flex items-center gap-4">
          ...
          <div><p class="text-slate-500 text-sm">Total Spent</p><p class="text-2xl font-bold">LKR <?= $total_spent_rs ?></p></div>
        </div>
      </div>
      <div class="card rounded-xl border border-border bg-gradient-to-br from-purple-50 to-white p-5">
        <div class="flex items-center gap-4">
          ...
          <div><p class="text-slate-500 text-sm">Completed Orders</p><p class="text-2xl font-bold"><?= $completed_orders ?></p></div>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full">
        ...
        <tbody class="divide-y divide-border bg-white text-sm">
          <?php if (!$orders): ?>
            <tr><td colspan="7" class="px-6 py-10 text-center text-slate-500">You haven’t purchased any materials yet.</td></tr>
          ...
          ?>
          <tr>
            <td class="px-6 py-4"><span class="text-primary font-medium">ORD-<?= str_pad((string)$oid, 3, '0', STR_PAD_LEFT) ?></span></td>
